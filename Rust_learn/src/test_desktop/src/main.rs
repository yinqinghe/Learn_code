use druid::widget::{Align,Flex,SizedBox,Label, TextBox,Button,Controller,ControllerHost,Scroll};
...
fn build_ui() -> impl Widget<AppData> {
    ...
    let output_list = SizedBox::new(output_list)
        ...
        .controller(OutputController);  // 设置背景颜色

    let output_list = Flex::row()
        .with_flex_child(Align::centered(output_list), 1.0);
    //创建一个连接按钮
    ...
    let ui = Flex::row()
        .with_flex_child(
            Flex::column()
                ...
                .with_flex_child(output_list, 1.0)
                .background(Color::WHITE),
            1.0,
        );
    // 创建Controller
...
fn main() -> Result<(), PlatformError> {
    ...
    let main_window = WindowDesc::new(build_ui)
        ...
        .window_size(Size::new(750.0, 650.0))  //(宽度，高度)
